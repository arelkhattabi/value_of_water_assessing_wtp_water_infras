#-------------------------------------------------------------------------------
#Results using only pre-IIJA data from 2021.
#Appendix E
#-------------------------------------------------------------------------------
...
model_con_trust_strict <- '
 ###########################
  ...
  wtp_value_strict ~ c1*trust + c2*satisfaction + c3*service_quality + income_before_tax_type +  urban_type + political_views_type
...
fits_sensitivity1 <- list(
  safety  = vow_surveys %>% filter(year==2021) %>% filter(wtp == "drinking water safety"),
  taste   = vow_surveys %>% filter(year==2021) %>% filter(wtp == "drinking water taste and smell"),
  poll    = vow_surveys %>% filter(year==2021) %>% filter(wtp == "infrasture/pollution"),
  afford  = vow_surveys %>% filter(year==2021) %>% filter(wtp == "help ensure water affordability for all")
)


latent_loadings_con_trust2021 <- get_latent_loadings(fits_sensitivity1, model_con_trust_strict, ordered_vars)

latent_variables_con_trust2021 <-  reduce(
  latent_loadings_con_trust2021,
  ...
  autofit() %>%
  width(width = 1.5) %>% 
  set_caption("Appendix Table E1. Latent Variable Construction Commitment Trust Theory--2021 Survey")

struct_list_sensitivity1_strict <- make_struct_list(
  fits = fits_sensitivity1,
  model = model_con_trust_strict,
  ordered_vars = ordered_vars
)

ft_struct_sensitivity2021 <- make_struct_table(struct_list_sensitivity1_strict) %>% 
  flextable() %>%  
  autofit() %>%
  width(width = 1.5) %>% 
  set_caption("Appendix Table E2. Results for Commitment Trust Theory using Strict Measure of WTP--2021 Survey")

# --- export to Word ---
doc_main_model_spec2021 <- read_docx() %>%
  body_add_flextable(latent_variables_con_trust2021) %>%
  body_add_par("") %>%
  body_add_flextable(ft_struct_sensitivity2021)  

print(doc_main_model_spec2021, target = "article submission/R script/tables5_appendix_e.docx")

rm(fits_sensitivity1,ft_struct_sensitivity2021,struct_list_sensitivity1_strict,
   latent_loadings_con_trust2021,latent_variables_con_trust2021,
   model_con_trust_strict, doc_main_model_spec2021)
